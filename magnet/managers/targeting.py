...
class Manager(tm.Manager[Module], Generic[Module]):
    ...
    * extends: `tm.Manager`
    ...
    def target(self, t: int) -> None:
        ...
        if isinstance(model, Targeting): model.target = t
    ...
    def target_dict(self) -> dict[int, str]:
        model = self.model.module if isinstance(self.model, torch.nn.parallel.DataParallel) else self.model
        if isinstance(model, Targeting): return model.target_dict
        else: return {}
    ...
    def _train(self, dataset: SizedIterable, show_verbose: bool = False, **kwargs: Any) -> dict[str, float]:
        ...
        if isinstance(dataset, Targeting) and self.target_freq == Frequency.EPOCH:
            for t in range(dataset.num_targets):
                ...
                if show_verbose: print(f"Training target {t} (Dataset {dataset.target_dict[t]})...")
                dataset.target = t
                self.target = t

                # train model
                try: subsummary = super()._train(dataset, show_verbose=show_verbose, **kwargs)
                except Exception as e: raise RuntimeError(f"Training dataset '{dataset.target_dict[t]}' failed") from e
                for k, v in subsummary.items(): summary[f'{k}_{dataset.target_dict[t]}'] = v
            return summary
        else: return super()._train(dataset, show_verbose=show_verbose, **kwargs)
    ...
    def test(self, dataset: SizedIterable, show_verbose: bool = False, **kwargs: Any) -> dict[str, float]:
        ...
        if not isinstance(dataset, dict): return super().test(dataset, show_verbose=show_verbose, **kwargs)

        # test datasets in one epoch
        for t, (name, d) in enumerate(dataset.items()):
            if show_verbose: print(f"Target {t} (Dataset {name}):")
            self.target = t
            try: subsummary = super().test(d, show_verbose=show_verbose, **kwargs)
    ...
    def train_step(self, x_train: torch.Tensor, y_train: Any) -> dict[str, float]:
        if self.target_freq == Frequency.EPOCH: return super().train_step(x_train, y_train)
        else:
            # initialize summary
            summary = {}
    ...
    def unpack_data(self, data: Any) -> tuple[Any, Union[dict[str, Any], Any]]:
        ...
        y_train = {
            "out": label,
            "domain": torch.tensor([self.target] * len(x_train))
        } if self.model.training is True else label
