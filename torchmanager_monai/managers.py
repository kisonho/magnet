...
class Manager(tm.Manager[Module], Generic[Module]):
    ...
    @torch.no_grad()
    def predict(self, dataset: Union[SizedIterable[Any], DataLoader[Any]], device: Optional[Union[torch.device, list[torch.device]]] = None, use_multi_gpus: bool = False, show_verbose: bool = False) -> list[torch.Tensor]:
    ...
    @torch.no_grad()
    def test(self, dataset: Union[SizedIterable[Any], DataLoader[Any]], device: Optional[torch.device] = None, use_multi_gpus: bool = False, show_verbose: bool = False, **kwargs: Any) -> dict[str, float]:
        # initialize
    ...
    def unpack_data(self, data: dict[str, Any]) -> tuple[torch.Tensor, Union[dict[str, torch.Tensor], torch.Tensor]]:
